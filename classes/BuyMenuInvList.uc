class BuyMenuInvList extends KFBuyMenuInvList;

function UpdateMyBuyables() {
    local class<KFVeterancyTypes> PlayerVeterancy;
    local KFPlayerReplicationInfo KFPRI;
    local array<GUIBuyable> fragBuyables;
    local GUIBuyable MyBuyable, KnifeBuyable, SecondaryAmmoBuyable;
    local Inventory CurInv;
    local KFLevelRules KFLR;
    local bool bHasDual, bHasDualCannon, bHasDual44, bhasDualM23;
    local float CurAmmo, MaxAmmo;
    local class<KFWeaponPickup> MyPickup, MyPrimaryPickup;
    local int DualDivider, NumInvItems, i;
    ...
    for ( CurInv = PlayerOwner().Pawn.Inventory; CurInv != none; CurInv = CurInv.Inventory ) {
        if ( KFWeapon(CurInv) != none ) {
            if ( KFWeapon(CurInv).default.PickupClass == class'DualDeaglePickup' ) {
                bHasDualCannon = true;
            } else if ( KFWeapon(CurInv).default.PickupClass == class'DualiesPickup' ) {
                bHasDual = true;
            } else if ( KFWeapon(CurInv).default.PickupClass == class'Dual44MagnumPickup' ) {
                bHasDual44 = true;
            } else if ( KFWeapon(CurInv).default.PickupClass == class'DualMK23Pickup' ) {
                bhasDualM23 = true;
            }
        }
    }
    ...
    for ( CurInv = PlayerOwner().Pawn.Inventory; CurInv != none; CurInv = CurInv.Inventory ) {
        ...
        if ( CurInv.IsA('KFWeapon') ) {
            ...
            if ( KFWeapon(CurInv).bHasSecondaryAmmo ) {
                ...
                MyBuyable.bSellable = false;
                fragBuyables[fragBuyables.Length]= MyBuyable;
            }
    ...
    for(i= 0; i < fragBuyables.Length; i++) {
        MyBuyables[9 + i] = FragBuyables[i];
    }
    MyBuyables[9 + i] = MyBuyable;
